 ...
 */
const test: {
  (props: SplitProps, result: ReactNode): void;
  (input: ReactNode, props: SplitProps, result: ReactNode): void;
  ...
  const actualResult = split(input, props).result;
  const expectedHTML = renderToHTML(result);
  expect(renderToDOM(actualResult).container).toContainHTML(expectedHTML);
  expect(renderToHTML(actualResult)).toBe(expectedHTML);
};
...
describe('content', () => {
  it('works', () => {
    const content = ((i, charOrWordOrWhitespace) => `${charOrWordOrWhitespace}_${i ?? '*'}`) as
      | ContentCallback_Chars
      | ContentCallback_Words;
