...
interface PropsBase {
  /**
   * The locale identifier(s) to use for segmentation.
   * @default 'en'
   */
  locale?: Intl.LocalesArgument;
}

interface CharsPropsBase extends PropsBase {
  /**
   * The splitting/segmentation method.
   */
...
interface WordsPropsBase extends PropsBase {
  /**
   * The splitting/segmentation method.
   */
...
export interface WordsProps_WithWhitespaceElements extends WordsPropsBase {
  /**
   * A callback to customize the properties for each character's element.
   * Note: It _will_ be called even if `by === Segmentation.WORDS` for whitespace characters.
   */
  charProps?: CharPropsCallback;
  /**
   * When `true`, the `<span>` element for each whitespace character will not be created.
